...
def has_prefix(s, prefix):
    ...
    while prefix != empty:
        if s == empty or first(s) != first(prefix):
            return False
        s = rest(s)
        prefix = rest(prefix)
    return True
...
def has_kitty_gene(dna):
    ...
    check = link('C', link('A', link('T',link('C', link('A', link('T'))))))
    
    return has_sublist(dna,check)
...
def count_change(amount, denominations):
    ...
    60
    """
    if amount == 0:                                                         # only 1 way to make sum ends the recursion
        return 1

    if amount < 0 or denominations == empty:                                # denom used is too big or no more smaller denoms left to use means no way so ends the recursion
        return 0
    
    included = count_change(amount - first(denominations), denominations)   # number of ways to make chage with the current denom
    excluded = count_change(amount, rest(denominations))                    # number of ways to make change without the current denom
